...
loopTurns:
	for round := 0; round < 1; /*req.Turns*/ round++ {
		...
		for i := 0; i < 4; i++ {
			...
			//fmt.Println(startY, "startY", topHalo, "topHalo")
			...
			//fmt.Println(topHalo, endY, len(slice))
			...
			calls[i] = clients[i].Go(stubs.IndependentGameOfLife, newReq, &resTemp, done)
		}
