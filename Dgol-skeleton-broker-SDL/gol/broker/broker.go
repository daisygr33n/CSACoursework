...
func (c *ConnectionBroker) CellsFlippedMethod(req stubs.Request, res *stubs.Response) (err error) {
	c.mu.Lock()
	res.Flipped = c.flipped
	res.CurrentTurn = c.currentTurn
	c.mu.Unlock()
	return
}
...
func calcFlipped(currentWorld, nextWorld [][]byte) []util.Cell {
	flipped := []util.Cell{}
	for y := range currentWorld {
		for x := range nextWorld[y] {
			if currentWorld[y][x] != nextWorld[y][x] {
...
func (c *ConnectionBroker) ParallelGolMethod(req stubs.Request, res *stubs.Response) (err error) {
	...
	if err != nil {
		fmt.Println("Failed to connect to server initially", err)
	}
	mu.Unlock()
...
loopTurns:
	for round := 0; round < req.Turns; round++ {
		...
		if localPaused { //busy waiting, keep checking paused until it becomes false
		loopPaused:
			for {
				mu.Lock()
				if !paused {
		...
		calls := make([]*rpc.Call, threads) //creating a slice to track client calls

		for i := 0; i < 4; i++ { //break up the image into rows (0-4, 4-8, 8-12, 12-16)
			...
			newReq := stubs.Request{
				...
				StartY:      startY, //use the previously calculated boundaries
				EndY:        endY,
				Height:      height,
				Width:       width,
				Turns:       req.Turns,
			...
			responses[i] = resTemp //asynchronous call to calculate gol logic on given row
			calls[i] = clients[i].Go(stubs.NextState, newReq, &resTemp, done)
		}

		for i := 0; i < 4; i++ {
			<-calls[i].Done //wait until each call is complete
		}
		...
		c.mu.Lock()
		flipped := calcFlipped(currentWorld, nextWorld)
		c.flipped = flipped
		c.aliveCells = aliveCells
		c.currentTurn++
		c.mu.Unlock()
